...
const mealCases = [
  ...
  "rajma chawal 350g",
  "idli sambar 250g",
  "masala dosa with sambar and chutney 320g",
  "chole bhature 300g restaurant",
  "poha with peanuts 250g",
  "upma 220g homemade",
  "chicken biryani 400g restaurant",
  "paneer tikka 220g",
  "2 aloo paratha with curd 300g",
  "fish curry rice 350g",
  "moong dal khichdi 320g",
  "palak paneer with 2 roti 350g",
];

const rows = mealCases.map((meal) => {
  const n = estimateMeal(meal);
  return {
    meal,
    kcal: Number(n.kcal.toFixed(0)),
    protein_g: Number(n.protein.toFixed(1)),
    carbs_g: Number(n.carbs.toFixed(1)),
    fat_g: Number(n.fat.toFixed(1)),
    fiber_g: Number(n.fiber.toFixed(1)),
    sugar_g: Number(n.sugar.toFixed(1)),
  };
});
...
const summary = {
  ...
  avgCalories: Number((rows.reduce((s, r) => s + r.kcal, 0) / rows.length).toFixed(1)),
  avgProtein: Number((rows.reduce((s, r) => s + r.protein_g, 0) / rows.length).toFixed(1)),
};
...
const outPath = path.join(root, "release", `calibration-20-meals-${new Date().toISOString().slice(0, 10)}.json`);
