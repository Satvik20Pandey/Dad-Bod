...
const AI_MODEL_GPT4O_MINI = "openai/gpt-4o-mini";
const AI_MODEL_GPT4O = "openai/gpt-4o";
const AI_MODEL_GEMINI_FLASH = "google/gemini-2.0-flash-001";
const AI_MODEL_CLAUDE_HAIKU = "anthropic/claude-3.5-haiku";
const DEFAULT_AI_MODEL = AI_MODEL_GPT4O_MINI;
const AI_MODEL_FALLBACKS = [
  AI_MODEL_GPT4O_MINI,
  AI_MODEL_GPT4O,
  AI_MODEL_GEMINI_FLASH,
  AI_MODEL_CLAUDE_HAIKU,
];
...
function openPrivacyPolicy() {
  openPolicyModal("Privacy Policy", `<div class="policy-content"><p><em>Last updated: June 2026</em></p><h3>Data Collection</h3><p>Dad Bod does not collect, store, or transmit any personal information to external servers. All user data including meal logs, workout records, weight entries, and photos are stored locally on your device using browser local storage.</p><h3>Third-Party Services</h3><p>If you choose to use AI features, your meal descriptions may be sent to OpenRouter API using your own API key. No personal identifiers are included.</p><h3>Cookies</h3><p>Dad Bod does not use cookies or any tracking technologies.</p><h3>Data Deletion</h3><p>You can delete all your data at any time by clearing your browser data or using the Clear Today / Export features in the app.</p><h3>Contact</h3><p>For privacy concerns: <a href="mailto:satvikofficial20@gmail.com">satvikofficial20@gmail.com</a></p></div>`);
}
...
function renderWorkoutProgress() {
  const workout = getEveningWorkoutForDay(currentDayName());
  const log = ensureGymLogForDate();
  const day = currentDayName();
  let total = absCircuit.length;
  let done = absCircuit.filter((_, i) => Boolean(log.exerciseDone?.[`abs-${i}`])).length;

  if (!workout.isOff) {
    total += workout.exercises.length;
    done += workout.exercises.filter((_, idx) => Boolean(log.exerciseDone?.[`${day}-${idx}`])).length;
  }

  const pct = total > 0 ? Math.round((done / total) * 100) : 0;
  const fill = select("workoutProgressFill");
    ...
    .map((ex, idx) => {
      ...
      return `
        <div class="exercise-item ${done ? "done" : ""}" onclick="toggleAbsExercise(${idx})">
          ...
          <div>
            <div class="exercise-index">Abs ${idx + 1}</div>
            <h3>${escapeHtml(ex.name)}</h3>
            <p class="exercise-meta">${escapeHtml(ex.sets)} · ${escapeHtml(ex.cues)}</p>
            <div class="exercise-actions">
              <button type="button" class="btn-small" onclick="event.stopPropagation(); openAbsGuide('${encodeURIComponent(ex.name)}')">Form Cues</button>
              <button type="button" class="btn-small btn-accent" onclick="event.stopPropagation(); startExerciseTimer('${encodeURIComponent(ex.name)}', ${timerSec})">${Math.round(timerSec)}s Timer</button>
            </div>
          </div>
        </div>
...
async function callOpenRouter(messages, modelOverride, options = {}) {
  ...
  const defaultMaxTokens = expectJson ? 480 : 320;
  const maxTokensResolved = Number.isFinite(Number(options.maxTokens)) ? Number(options.maxTokens) : defaultMaxTokens;
  const maxTokens = maxTokensResolved;
  const totalTimeoutMs = Number.isFinite(Number(options.totalTimeoutMs)) ? Number(options.totalTimeoutMs) : 18000;
  ...
  for (const model of modelChain) {
    ...
    const requestTimeout = Math.max(1200, Math.min(8000, remaining - 120));
...
async function startVoiceInput(targetInputId = "mealDescription") {
  ...
  recog.onresult = (event) => {
    ...
    if (preview) {
      input.value = preview;
      setText("mealStatus", `Listening... ${preview}`);
    }
  };
...
function renderApiSettings() {
  ...
  if (modelSelect) {
    ...
    const legacyMap = {
      "stepfun-ai/step-3.5-flash": AI_MODEL_GPT4O_MINI,
      "qwen/qwen3-next-80b-a3b-instruct": AI_MODEL_GPT4O,
      "nvidia/llama-3.1-nemotron-nano-8b-v1": AI_MODEL_GPT4O_MINI,
    };
    const normalizedModel = legacyMap[preferredModel] || preferredModel;
    const hasOption = Array.from(modelSelect.options || []).some((option) => option.value === normalizedModel);
    modelSelect.value = hasOption ? normalizedModel : DEFAULT_AI_MODEL;
    if (normalizedModel !== preferredModel) {
      state.settings.aiModel = modelSelect.value;
      saveState();
    }
  }
...
function getExerciseVideoQuery(exerciseName) {
  return `${exerciseName} proper form gym technique`;
}

function openExerciseModal(exercise) {
  setText("modalTitle", exercise.name);
  setText("modalSets", exercise.sets || "—");
  ...
  if (videoBtn) {
    videoBtn.dataset.query = exercise.videoQuery || getExerciseVideoQuery(exercise.name);
  }
...
function openModalVideoGuide() {
  const query = select("modalVideoBtn")?.dataset?.query || "exercise proper form";
...
function bindAppEvents() {
  ...
  select("settingsRate")?.addEventListener("click", () => {
    showToast("Thank you. Rating will be available on the Play Store soon.");
  });
